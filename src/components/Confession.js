...
//TODO: there is an error here/in MaterialIcon that causes a memory leak/no op.
//I'm pretty sure it is because MaterialIcon fetches the image dynamically,
// so if an unmount occurs, there is a memory leak/no-op
    ...
    <StyledConfession>
      <Header>
        ...
        <Row>
          ...
          <StyledInfo style={{ marginRight: "10px" }}>{props.date}</StyledInfo>
          <StyledMaterialIcon icon="person" />
          <StyledInfo>{props.reacts.Total}</StyledInfo>
        </Row>
      </Header>
      <WordWrappedParagraph>{props.text}</WordWrappedParagraph>
    </StyledConfession>
  </DestyledLink>
);
